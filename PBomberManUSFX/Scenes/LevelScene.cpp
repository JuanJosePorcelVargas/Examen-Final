...
void LevelScene::spawnGrass(const int positionX, const int positionY)
{
    ...
    addObject(grass);
    normal.push_back(std::make_pair(GameTile::Grass, grass));//******************************************
    backgroundObjectLastNumber++;
}
//********************************************************************************************
void LevelScene::spawnHielo(const int positionX, const int positionY)
{
    auto hielo = std::make_shared<soilHielo>(gameManager->getAssetManager()->getTexture(GameTexture::Hielo), gameManager->getRenderer());
    hielo->setPosition(positionX, positionY);
    hielo->setSize(scaledTileSize, scaledTileSize);
    addObject(hielo);
    lento.push_back(std::make_pair(GameTile::Hielo, hielo));
};
//*********************************************************************************************
...
void LevelScene::updatePlayerCollision()//********************
{
    ...
    //************************************************************************************
    for (const auto& collisionIceObject : lento)
    {
        if (isCollisionDetected(playerRect, collisionIceObject.second->getRect()))
        {
            if (gameVersion == GameVersion::GAMEVERSION_CARTOON) {
                player->ColisionHielo(0.003f);
            }
            else if (gameVersion == GameVersion::GAMEVERSION_CLASIC) {
                player->ColisionHielo(0.005f);
            }
            else if (gameVersion == GameVersion::GAMEVERSION_CUSTOM) {
                player->ColisionHielo(0.0065f);
            }
            else if (gameVersion == GameVersion::GAMEVERSION_REALISTIC) {
                player->ColisionHielo(0.0075f);
            }
            
        }
    }
    for (const auto& noSlowCollisionObject : normal)
    {
        if (isCollisionDetected(playerRect, noSlowCollisionObject.second->getRect()))
        {
            if (gameVersion == GameVersion::GAMEVERSION_CARTOON) {
                player->NoColision(0.006f);
            }
            else if (gameVersion == GameVersion::GAMEVERSION_CLASIC) {
                player->NoColision(0.01f);
            }
            else if (gameVersion == GameVersion::GAMEVERSION_CUSTOM) {
                player->NoColision(0.013f);
            }
            else if (gameVersion == GameVersion::GAMEVERSION_REALISTIC) {
                player->NoColision(0.015f);
            }
        }
    }
    //******************************************************************************************
    		...
    		if (chars[i] != ' ') {
    			...
    			case '2':
                    ...
                    //spawnWallPacman(fieldPositionX + x * scaledTileSize, fieldPositionY + y * scaledTileSize, tile);
       				break;
                case '3':
                    spawnHielo(fieldPositionX + x * scaledTileSize, fieldPositionY + y * scaledTileSize);//***********************************************
                    break;
    			}
                x++;
    		}
