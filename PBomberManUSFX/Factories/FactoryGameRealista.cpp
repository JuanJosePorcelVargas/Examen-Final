...
{
    // spawn player
    auto player = std::make_shared<RealistaPlayer>(gameManager->getAssetManager()->getTexture(GameTexture::Player), gameManager->getRenderer());
    ...
    std::cout << "Dentro FactoryGameRealista" << endl;
...
std::shared_ptr<GameObject> FactoryGameRealista::CreateEnemy(AIType _type, const int _positionX, const int _positionY)
{
    ...
    auto enemy = std::make_shared<RealistaEnemy>(gameManager->getAssetManager()->getTexture(GameTexture::Enemy1), gameManager->getRenderer());
